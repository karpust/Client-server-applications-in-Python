...
THR1.start()
THR2.start()
# присоединение основного потока к потоку 1:
THR1.join()
# первый поток запускается вместе с основным, основной ждет его завершения и завершается сам
# THR2.join()
end = time.time()
...
# через секунду заканчивается второй поток
