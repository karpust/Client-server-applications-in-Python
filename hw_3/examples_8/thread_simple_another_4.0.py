...
def writer(mes, event_for_wait, event_for_set):
    ...
    for i in range(10):
        ...
        event_for_wait.wait()  # останавливается ждать пока событие не станет True
        # сбросить флаг события c True на False
        event_for_wait.clear()  # True меняет на False, а False не трогает
        # выводим параметр
        print(f'{i} - {mes}')
        # устанавливаем флага события в True
        # потоки, которые его ожидают, активизируются
        event_for_set.set()  # False меняет на True, а True не трогает
